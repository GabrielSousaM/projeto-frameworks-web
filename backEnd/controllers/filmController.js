  ...
  try {
    ...
    if (Array.isArray(atores) && atores.length > 0) {
      ...
      for (const name of atores) {
        const [ator] = await Ator.findOrCreate({
          where: { name },
          defaults: { name },
          transaction: t,
        });
    ...
    const filmWithAtores = await Film.findByPk(film.id, {
...
async function updateFilm(req, res, next) {
  ...
  try {
    ...
    if (Array.isArray(atorIds)) {
      const found = await Ator.findAll({
        where: { id: atorIds },
        transaction: t,
      });
    ...
    const updated = await Film.findByPk(film.id, {
      include: { model: Ator, as: "atores", through: { attributes: [] } },
    });
    res.json(updated);
  } catch (err) {
